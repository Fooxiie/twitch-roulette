<x-guest-layout>
    <div
        class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 "
        style="background-color: #5A0C14;background-image: url('data:image/svg+xml,%3Csvg width=\'180\' height=\'180\' viewBox=\'0 0 180 180\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cpath d=\'M81.28 88H68.413l19.298 19.298L81.28 88zm2.107 0h13.226L90 107.838 83.387 88zm15.334 0h12.866l-19.298 19.298L98.72 88zm-32.927-2.207L73.586 78h32.827l.5.5 7.294 7.293L115.414 87l-24.707 24.707-.707.707L64.586 87l1.207-1.207zm2.62.207L74 80.414 79.586 86H68.414zm16 0L90 80.414 95.586 86H84.414zm16 0L106 80.414 111.586 86h-11.172zm-8-6h11.173L98 85.586 92.414 80zM82 85.586L87.586 80H76.414L82 85.586zM17.414 0L.707 16.707 0 17.414V0h17.414zM4.28 0L0 12.838V0h4.28zm10.306 0L2.288 12.298 6.388 0h8.198zM180 17.414L162.586 0H180v17.414zM165.414 0l12.298 12.298L173.612 0h-8.198zM180 12.838L175.72 0H180v12.838zM0 163h16.413l.5.5 7.294 7.293L25.414 172l-8 8H0v-17zm0 10h6.613l-2.334 7H0v-7zm14.586 7l7-7H8.72l-2.333 7h8.2zM0 165.414L5.586 171H0v-5.586zM10.414 171L16 165.414 21.586 171H10.414zm-8-6h11.172L8 170.586 2.414 165zM180 163h-16.413l-7.794 7.793-1.207 1.207 8 8H180v-17zm-14.586 17l-7-7h12.865l2.333 7h-8.2zM180 173h-6.613l2.334 7H180v-7zm-21.586-2l5.586-5.586 5.586 5.586h-11.172zM180 165.414L174.414 171H180v-5.586zm-8 5.172l5.586-5.586h-11.172l5.586 5.586zM152.933 25.653l1.414 1.414-33.94 33.942-1.416-1.416 33.943-33.94zm1.414 127.28l-1.414 1.414-33.942-33.94 1.416-1.416 33.94 33.943zm-127.28 1.414l-1.414-1.414 33.94-33.942 1.416 1.416-33.943 33.94zm-1.414-127.28l1.414-1.414 33.942 33.94-1.416 1.416-33.94-33.943zM0 85c2.21 0 4 1.79 4 4s-1.79 4-4 4v-8zm180 0c-2.21 0-4 1.79-4 4s1.79 4 4 4v-8zM94 0c0 2.21-1.79 4-4 4s-4-1.79-4-4h8zm0 180c0-2.21-1.79-4-4-4s-4 1.79-4 4h8z\' fill=\'%23f5f4f7\' fill-opacity=\'0.40\' fill-rule=\'evenodd\'/%3E%3C/svg%3E');">

        @if (!$participant)
            <button id="btn_participate" onclick="SitAtTable()" class="m-auto
            text-center
                    absolute left-1/2 top-4 w-auto p-4 mt-4
                    bg-gray-400 font-bold text-white border-gray-200 border-2">
                M'assoir
            </button>
        @endif

        <svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" width="90%"
             height="90%"
             style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd"
             viewBox="0 0 29900 13285"
             xmlns:xlink="http://www.w3.org/1999/xlink">
             <defs>
                 <style type="text/css">
                     .fil5 {
                         fill: #FEFEFE
                     }

                     .noir {
                         fill: #2B2A29
                     }

                     .noir:hover {
                         fill: #62615e
                     }

                     .rouge {
                         fill: #E31E24
                     }

                     .rouge:hover {
                         fill: #721012
                     }

                     .fil16 {
                         fill: #00A0E3
                     }

                     .fil1 {
                         fill: #287513
                     }

                     .fil4 {
                         fill: #2CA50C
                     }

                     .fil4:hover {
                         fill: #1d7508
                     }

                     .fil9 {
                         fill: #FEFEFE;
                         fill-rule: nonzero
                     }

                     .fil8 {
                         fill: #FEFEFE;
                         fill-rule: nonzero
                     }

                     .fil6 {
                         fill: #FEFEFE;
                         fill-rule: nonzero
                     }

                     .bois {
                         fill: #6F3812;
                         fill-rule: nonzero
                     }
                 </style>
             </defs>
            <g id="Layer_x0020_1">
                <metadata id="CorelCorpID_0Corel-Layer"/>
                <g id="_1114945664">
                    <g id="parent">
                        <path class="bois shadow-2xl"
                              d="M4121 1270c546,-146 1110,-184 1675,-184l23294 0c446,0 810,365 810,810l0 9500c0,445 -364,810 -810,810l-23775 -11c-884,-1 -1752,-290 -2534,-741 -1281,-740 -2209,-1945 -2592,-3374 -381,-1427 -181,-2940 557,-4218 736,-1275 1950,-2210 3375,-2592z"/>
                        <path class="fil1"
                              d="M5589 1570l23371 0c275,0 499,225 499,500l0 9139c0,274 -224,499 -499,499l-23369 0c-274,0 -47,-225 -47,-499l0 -9139c0,-275 -230,-500 45,-500z"/>

                        <polygon class="noir" onclick="chooseTile(this)"
                                 id="noire"
                                 points="20339,10086 22628,10086 22628,8745 20339,8745 "/>
                        <polygon class="rouge" onclick="chooseTile(this)"
                                 id="rouge"
                                 points="17956,10086 17956,8745 20293,8745 20293,10086 "/>
                        <path class="fil4" onclick="chooseTile(this)" id="0"
                              d="M12056 3902c32,-79 65,-157 100,-234 27,-60 55,-119 87,-176 24,-44 51,-87 82,-126 34,-43 72,-79 120,-105 64,-36 137,-49 209,-55 47,-4 94,-5 141,-5 58,-1 116,0 174,1 53,0 105,1 158,1l20 0 0 4370 -20 0c-53,0 -105,1 -158,2 -58,0 -116,1 -174,1 -47,-1 -94,-2 -141,-6 -72,-6 -145,-19 -209,-55 -48,-26 -86,-62 -120,-105 -31,-39 -58,-82 -82,-125 -32,-58 -60,-117 -87,-176 -35,-78 -68,-156 -100,-234 -42,-100 -82,-200 -122,-300 -70,-173 -140,-346 -211,-518 -91,-221 -185,-441 -280,-661l-4 -8 4 -8c95,-219 189,-440 280,-661 71,-172 141,-345 211,-518 40,-100 80,-200 122,-299z"/>
                        <g>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="1"
                                     points="13214,6157 13214,7553 14351,7553 14351,6157 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="3"
                                     points="13214,4628 14351,4628 14351,3224 13214,3224 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="5"
                                     points="14437,4715 14437,6070 15530,6070 15530,4715 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="7"
                                     points="15617,6157 15617,7553 16710,7553 16710,6157 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="9"
                                     points="15617,4628 16710,4628 16710,3224 15617,3224 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="12"
                                     points="16796,3224 16796,4628 17889,4628 17889,3224 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="14"
                                     points="17976,4715 17976,6070 19069,6070 19069,4715 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="16"
                                     points="19155,6157 19155,7553 20248,7553 20248,6157 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="18"
                                     points="19155,4628 20248,4628 20248,3224 19155,3224 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="21"
                                     points="20335,3224 20335,4628 21428,4628 21428,3224 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="19"
                                     points="21428,6157 20335,6157 20335,7553 21428,7553 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="23"
                                     points="21514,4715 21514,6070 22607,6070 22607,4715 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="27"
                                     points="22694,3224 22694,4628 23787,4628 23787,3224 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="30"
                                     points="23873,3224 23873,4628 24966,4628 24966,3224 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="36"
                                     points="26232,3224 26232,4628 27325,4628 27325,3224 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="32"
                                     points="26146,4715 25053,4715 25053,6070 26146,6070 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="34"
                                     points="27325,6157 26232,6157 26232,7553 27325,7553 "/>
                            <polygon class="rouge" onclick="chooseTile(this)"
                                     id="25"
                                     points="23787,7553 23787,6157 22694,6157 22694,7553 "/>
                        </g>
                        <g>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="4"
                                     points="15530,7553 15530,6157 14437,6157 14437,7553 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="2"
                                     points="13214,6070 14351,6070 14351,4715 13214,4715 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="6"
                                     points="14437,3224 14437,4628 15530,4628 15530,3224 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="8"
                                     points="15617,4715 15617,6070 16710,6070 16710,4715 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="10"
                                     points="16796,6157 16796,7553 17889,7553 17889,6157 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="11"
                                     points="16796,6070 17889,6070 17889,4715 16796,4715 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="15"
                                     points="17976,4628 19069,4628 19069,3224 17976,3224 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="13"
                                     points="19069,6157 17976,6157 17976,7553 19069,7553 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="17"
                                     points="19155,4715 19155,6070 20248,6070 20248,4715 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="20"
                                     points="20335,4715 20335,6070 21428,6070 21428,4715 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="24"
                                     points="21514,3224 21514,4628 22607,4628 22607,3224 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="22"
                                     points="22607,6157 21514,6157 21514,7553 22607,7553 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="26"
                                     points="22694,4715 22694,6070 23787,6070 23787,4715 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="28"
                                     points="23873,4715 23873,6070 24966,6070 24966,4715 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="33"
                                     points="25053,3224 25053,4628 26146,4628 26146,3224 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="28"
                                     points="24966,6157 23873,6157 23873,7553 24966,7553 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="31"
                                     points="25053,6157 25053,7553 26146,7553 26146,6157 "/>
                            <polygon class="noir" onclick="chooseTile(this)"
                                     id="35"
                                     points="26232,6070 27325,6070 27325,4715 26232,4715 "/>
                        </g>
                        <g>
                            <polygon class="fil4" onclick="chooseTile(this)"
                                     id="2/12"
                                     points="17970,8682 17970,7637 22610,7637 22610,8682 "/>
                            <polygon class="fil4" onclick="chooseTile(this)"
                                     id="1/12"
                                     points="13199,8682 13199,7637 17900,7637 17900,8682 "/>
                            <polygon class="fil4" onclick="chooseTile(this)"
                                     id="3/12"
                                     points="22690,8682 22690,7637 27332,7637 27332,8682 "/>
                            <polygon class="fil4" onclick="chooseTile(this)"
                                     id="1/18"
                                     points="13599,10070 13399,10030 13199,9800 13199,8762 15530,8762 15530,10070 "/>
                            <polygon class="fil4" onclick="chooseTile(this)"
                                     id="even"
                                     points="15610,10070 15610,8762 17893,8762 17893,10070 "/>
                            <polygon class="fil4" onclick="chooseTile(this)"
                                     id="odd"
                                     points="22694,10070 22694,8762 24966,8762 24966,10070 "/>
                            <polygon class="fil4" onclick="chooseTile(this)"
                                     id="19/36"
                                     points="25050,10070 25050,8762 27328,8762 27328,9870 27110,10067 "/>
                        </g>

                        <g>
                            <polygon class="fil4" onclick="chooseTile(this)"
                                     id="2:1/34"
                                     points="27410,3214 27410,4630 28525,4628 28525,4628 28545,3424 28345,3224"/>
                            <polygon class="fil4" onclick="chooseTile(this)"
                                     id="2:1/35"
                                     points="27410,4715 27410,6070 28545,6070 28545,4715"/>
                            <polygon class="fil4" onclick="chooseTile(this)"
                                     id="2:1/36"
                                     points="27410,6157 27410,7553 28545,7553 28345,7553 28545,7243 28545,6157"/>
                        </g>

                        <path class="fil6" id="text-0"
                              onclick="chooseTile(this)"
                              d="M12075 5387c0,159 142,236 320,236 175,-2 318,-71 318,-230 0,-158 -131,-235 -322,-235 -169,0 -316,64 -316,229zm109 2c0,-58 72,-87 209,-87 140,0 211,30 211,87 0,55 -68,89 -209,89 -143,0 -211,-35 -211,-89z"/>
                        <polygon class="fil8" id="text-1"
                                 onclick="chooseTile(this)"
                                 points="14018,6853 14018,6747 13547,6747 13547,6838 13604,6963 13687,6945 13644,6854 13644,6853 "/>
                        <path class="fil8" id="text-2"
                              onclick="chooseTile(this)"
                              d="M14022 5225l-91 0 0 178 -1 0 -37 -44c-60,-68 -123,-126 -203,-126 -85,0 -147,60 -147,166 0,64 21,119 48,155l79 -31c-19,-25 -39,-61 -39,-102 0,-54 30,-77 69,-77 55,2 108,51 200,155l56 61 66 0 0 -335 0 0z"/>
                        <path class="fil8" id="text-3"
                              onclick="chooseTile(this)"
                              d="M13993 4093c17,-28 33,-80 33,-141 0,-119 -60,-193 -145,-193 -61,0 -104,46 -114,103l-1 0c-20,-59 -60,-87 -110,-87 -65,0 -118,57 -118,157 0,61 18,118 36,146l80 -22c-12,-20 -29,-61 -29,-99 0,-47 21,-70 50,-70 39,0 53,47 54,84l0 43 80 0 0 -45c0,-48 20,-95 67,-95 35,0 64,29 64,86 0,45 -20,91 -30,110l83 23 0 0z"/>
                        <path class="fil8" id="text-4"
                              onclick="chooseTile(this)"
                              d="M15220 6725l-113 0 0 -56 -82 0 0 56 -277 0 0 136 288 179 71 0 0 -210 113 0 0 -105 0 0zm-195 212l-2 0 -103 -64c-30,-16 -57,-28 -88,-43l0 -3c31,2 60,3 88,3l105 0 0 107 0 0z"/>
                        <path class="fil8" id="text-5"
                              onclick="chooseTile(this)"
                              d="M14744 5245l0 259 247 31c-3,-19 -4,-37 -4,-63 0,-95 30,-133 79,-133 50,0 71,49 71,95 0,42 -13,85 -23,106l83 19c14,-26 27,-73 27,-130 0,-128 -78,-203 -165,-203 -56,0 -95,22 -119,57 -24,33 -34,79 -34,125 0,11 0,20 1,30l-72 -10 0 -183 -91 0 0 0z"/>
                        <path class="fil8" id="text-6"
                              onclick="chooseTile(this)"
                              d="M14741 3796c-1,11 -1,25 0,45 4,74 27,135 70,183 48,50 121,83 210,83 113,0 206,-61 206,-188 0,-102 -77,-174 -170,-174 -98,0 -154,67 -154,146 0,44 14,77 37,100l0 2c-54,-12 -105,-55 -113,-153 -2,-17 -2,-31 -2,-44l-84 0 0 0zm405 125c0,50 -45,74 -95,76 -13,0 -22,-2 -28,-5 -24,-10 -42,-35 -42,-65 0,-47 37,-70 82,-70 48,0 83,25 83,64l0 0z"/>
                        <polygon class="fil8" id="text-7"
                                 onclick="chooseTile(this)"
                                 points="15927,7021 16019,7021 16019,6805 16020,6805 16400,6999 16400,6883 15998,6688 15927,6688 "/>
                        <path class="fil8" id="text-8"
                              onclick="chooseTile(this)"
                              d="M16408 5395c0,-110 -58,-181 -144,-181 -60,0 -100,40 -118,87l-2 0c-23,-48 -63,-69 -103,-69 -58,0 -122,45 -122,156 0,94 53,167 133,167 42,0 80,-23 103,-68l2 0c23,51 63,84 119,84 64,0 132,-55 132,-176l0 0zm-77 -4c0,41 -32,67 -66,67 -34,0 -60,-21 -70,-58 12,-43 36,-73 77,-73 34,0 59,25 59,64l0 0zm-336 2c0,-40 29,-57 57,-57 30,0 53,23 60,48 -11,37 -31,66 -63,66 -30,0 -54,-19 -54,-57l0 0z"/>
                        <path class="fil8" id="text-9"
                              onclick="chooseTile(this)"
                              d="M16406 4061c1,-12 2,-31 0,-51 -3,-73 -25,-134 -65,-179 -48,-52 -120,-83 -217,-83 -111,0 -204,57 -204,178 0,105 78,178 169,178 92,0 148,-67 148,-147 0,-43 -12,-73 -34,-94l1 -2c35,8 66,29 86,59 19,25 28,59 30,92 2,23 2,35 0,49l86 0 0 0zm-406 -132c0,-50 50,-68 99,-68 15,0 22,2 29,5 17,11 31,31 31,61 0,45 -35,67 -76,67 -49,0 -83,-27 -83,-65z"/>
                        <path class="fil8" id="text-10"
                              onclick="chooseTile(this)"
                              d="M17579 7105l0 -107 -472 0 0 91 58 126 82 -18 -43 -91 0 -1 375 0zm-480 -435c0,122 108,180 244,180 134,-1 244,-54 244,-177 0,-120 -100,-179 -246,-179 -130,0 -242,50 -242,176zm83 1c0,-44 55,-66 160,-66 107,0 161,23 161,65 0,43 -52,70 -160,69 -110,1 -161,-27 -161,-68z"/>
                        <path class="fil8" id="text-11"
                              onclick="chooseTile(this)"
                              d="M17579 5593l0 -107 -472 0 0 91 58 125 82 -18 -43 -89 0 -2 375 0zm0 -403l0 -107 -472 0 0 91 58 125 82 -18 -43 -89 0 -2 375 0z"/>
                        <path class="fil8" id="text-12"
                              onclick="chooseTile(this)"
                              d="M17583 4169l0 -107 -472 0 0 92 58 125 82 -19 -43 -89 0 -2 375 0zm0 -596l-91 0 0 178 -1 0 -37 -45c-61,-68 -124,-125 -204,-125 -85,0 -147,59 -147,166 0,64 22,118 48,154l79 -31c-19,-25 -39,-60 -39,-101 0,-55 30,-78 69,-78 56,1 108,52 200,155l56 61 67 0 0 -334 0 0z"/>
                        <path class="fil8" id="text-13"
                              onclick="chooseTile(this)"
                              d="M18758 7096l0 -107 -471 0 0 91 57 126 83 -18 -43 -91 0 -1 374 0zm-25 -259c17,-28 33,-79 33,-141 0,-119 -61,-193 -145,-193 -61,0 -104,46 -114,103l-1 0c-20,-58 -60,-87 -110,-87 -65,0 -117,56 -117,157 0,62 17,118 35,146l80 -22c-12,-20 -29,-60 -29,-100 0,-46 21,-69 49,-69 40,0 55,47 56,84l0 44 78 0 0 -46c0,-49 21,-95 68,-95 35,0 63,29 63,86 0,46 -18,91 -29,110l83 23 0 0z"/>
                        <path class="fil8" id="text-14"
                              onclick="chooseTile(this)"
                              d="M18758 5647l0 -107 -471 0 0 92 57 124 83 -18 -43 -89 0 -2 374 0zm0 -562l-113 0 0 -56 -82 0 0 56 -276 0 0 136 287 178 71 0 0 -209 113 0 0 -105 0 0zm-195 211l-2 0 -102 -63c-31,-16 -58,-28 -89,-44l0 -2c31,2 60,3 89,3l104 0 0 106 0 0z"/>
                        <path class="fil8" id="text-15"
                              onclick="chooseTile(this)"
                              d="M18754 4169l0 -106 -471 0 0 91 57 125 83 -18 -43 -90 0 -2 374 0zm-471 -577l0 260 247 31c-3,-19 -4,-37 -4,-63 0,-95 29,-133 78,-133 51,0 72,48 72,94 0,43 -13,86 -24,107l83 19c14,-26 27,-73 27,-132 0,-126 -77,-202 -166,-202 -54,0 -93,22 -117,58 -25,33 -35,79 -35,124 0,11 0,20 1,30l-72 -10 0 -183 -90 0 0 0z"/>
                        <path class="fil8" id="text-16"
                              onclick="chooseTile(this)"
                              d="M19938 7107l0 -107 -472 0 0 92 58 125 82 -18 -43 -91 0 -1 375 0zm-479 -563c0,11 0,24 1,44 4,74 27,134 69,182 47,51 122,84 210,84 113,0 206,-62 206,-188 0,-102 -76,-174 -170,-174 -98,0 -155,67 -155,146 0,44 14,77 39,101l0 2c-56,-13 -106,-57 -114,-154 -1,-18 -2,-31 -1,-43l-85 0 0 0zm406 124c0,49 -46,74 -96,76 -13,0 -22,-1 -28,-4 -23,-12 -41,-36 -41,-66 0,-47 37,-70 80,-70 48,0 85,25 85,64l0 0z"/>
                        <path class="fil8" id="text-17"
                              onclick="chooseTile(this)"
                              d="M19938 5638l0 -107 -472 0 0 92 58 124 82 -18 -42 -89 0 -2 374 0zm-472 -268l91 0 0 -216 1 0 380 195 0 -117 -402 -194 -70 0 0 332z"/>
                        <path class="fil8" id="text-18"
                              onclick="chooseTile(this)"
                              d="M19938 4177l0 -107 -472 0 0 91 58 126 82 -18 -42 -91 0 -1 374 0zm8 -430c0,-109 -57,-182 -144,-182 -60,0 -99,41 -117,87l-2 0c-23,-47 -64,-69 -104,-69 -57,0 -121,47 -121,157 0,95 53,166 132,166 42,0 81,-22 104,-68l2 0c23,52 62,84 118,84 65,0 132,-55 132,-175l0 0zm-76 -4c0,41 -32,67 -67,66 -34,0 -60,-21 -70,-57 12,-43 36,-74 77,-74 35,0 60,26 60,65l0 0zm-337 2c0,-41 29,-57 57,-57 30,0 54,22 61,47 -11,38 -32,67 -64,67 -30,0 -54,-20 -54,-57l0 0z"/>
                        <path class="fil8" id="text-19"
                              onclick="chooseTile(this)"
                              d="M21118 7105l0 -107 -472 0 0 92 58 124 82 -18 -42 -89 0 -2 374 0zm6 -297c1,-13 2,-31 0,-50 -3,-73 -25,-136 -66,-180 -48,-52 -119,-83 -217,-83 -111,0 -203,57 -203,179 0,103 77,178 169,178 92,0 148,-68 148,-148 0,-42 -13,-74 -34,-95l1 -2c35,9 65,29 86,60 18,26 28,60 30,93 2,22 2,34 0,48l86 0 0 0zm-406 -131c0,-51 49,-70 99,-70 14,0 22,2 29,6 16,10 31,32 31,62 0,44 -35,66 -76,66 -50,1 -83,-27 -83,-64z"/>
                        <path class="fil8" id="text-20"
                              onclick="chooseTile(this)"
                              d="M21118 5434l-92 0 0 178 -1 0 -36 -44c-61,-68 -125,-125 -204,-125 -86,0 -148,58 -148,165 0,64 22,119 48,154l80 -31c-19,-24 -39,-60 -39,-100 0,-55 31,-79 69,-79 55,2 107,53 200,155l55 61 68 0 0 -334 0 0zm-481 -241c0,121 109,179 245,179 133,0 244,-54 244,-175 0,-121 -101,-180 -247,-180 -129,0 -242,49 -242,176zm84 2c0,-46 55,-68 159,-68 108,0 161,23 161,67 0,42 -51,68 -159,68 -110,0 -161,-27 -161,-67z"/>
                        <path class="fil8" id="text-21"
                              onclick="chooseTile(this)"
                              d="M21122 3917l-92 0 0 178 -1 0 -36 -44c-61,-69 -125,-126 -204,-126 -86,0 -148,59 -148,166 0,64 22,119 48,155l80 -31c-19,-26 -39,-61 -39,-102 0,-54 30,-77 69,-77 55,2 107,51 200,154l55 62 68 0 0 -335 0 0zm0 -209l0 -108 -473 0 0 92 58 125 83 -18 -43 -90 0 -1 375 0z"/>
                        <path class="fil8" id="text-22"
                              onclick="chooseTile(this)"
                              d="M22301 6889l-90 0 0 178 -2 0 -37 -44c-61,-68 -123,-125 -203,-125 -85,0 -148,58 -148,166 0,63 22,118 49,154l79 -32c-19,-24 -39,-60 -39,-100 0,-55 30,-78 68,-78 56,1 109,52 201,155l56 60 66 0 0 -334 0 0zm0 -403l-90 0 0 178 -2 0 -37 -44c-61,-68 -123,-125 -203,-125 -85,0 -148,58 -148,166 0,63 22,118 49,154l79 -32c-19,-24 -39,-60 -39,-100 0,-55 30,-78 68,-78 56,1 109,52 201,155l56 60 66 0 0 -334 0 0z"/>
                        <path class="fil8" id="text-23"
                              onclick="chooseTile(this)"
                              d="M22297 5425l-91 0 0 178 -2 0 -36 -44c-60,-68 -124,-125 -203,-125 -86,0 -148,58 -148,165 0,64 21,119 48,154l80 -31c-19,-24 -39,-60 -39,-100 0,-55 30,-79 68,-79 55,2 108,53 201,155l55 61 67 0 0 -334 0 0zm-26 -66c17,-28 34,-79 34,-140 0,-120 -62,-193 -145,-193 -61,0 -105,46 -115,102l-1 0c-21,-58 -60,-86 -109,-86 -66,0 -118,56 -118,157 0,61 17,117 35,145l80 -22c-11,-19 -28,-60 -28,-98 0,-47 21,-71 49,-71 40,0 54,48 55,85l0 43 78 0 0 -45c0,-49 22,-96 69,-96 35,0 62,29 62,87 0,45 -18,90 -29,109l83 23 0 0z"/>
                        <path class="fil8" id="text-24"
                              onclick="chooseTile(this)"
                              d="M22301 3971l-90 0 0 178 -2 0 -37 -43c-61,-68 -123,-126 -203,-126 -85,0 -148,59 -148,165 0,64 22,120 49,155l79 -31c-19,-24 -39,-61 -39,-101 0,-55 30,-78 68,-78 56,2 109,52 201,155l56 61 66 0 0 -335 0 0zm0 -369l-113 0 0 -56 -82 0 0 56 -277 0 0 136 288 179 71 0 0 -209 113 0 0 -106 0 0zm-195 211l-2 0 -102 -63c-31,-16 -58,-28 -89,-43l0 -3c31,2 59,4 89,4l104 0 0 105 0 0z"/>
                        <path class="fil8" id="text-25"
                              onclick="chooseTile(this)"
                              d="M23476 6888l-90 0 0 178 -2 0 -37 -43c-60,-68 -123,-126 -203,-126 -85,0 -147,59 -147,165 0,64 21,120 48,155l79 -31c-19,-24 -39,-61 -39,-101 0,-55 30,-78 68,-78 56,2 109,52 201,155l56 61 66 0 0 -335 0 0zm-471 -384l0 261 247 31c-3,-19 -4,-38 -4,-65 0,-93 29,-132 78,-132 50,0 72,49 72,94 0,43 -13,86 -24,107l83 20c14,-26 27,-74 27,-132 0,-126 -77,-202 -166,-202 -54,0 -93,21 -117,58 -25,33 -35,78 -35,123 0,12 0,20 1,31l-72 -11 0 -183 -90 0 0 0z"/>
                        <path class="fil8" id="text-26"
                              onclick="chooseTile(this)"
                              d="M23476 5436l-90 0 0 178 -2 0 -37 -44c-60,-68 -123,-125 -203,-125 -85,0 -147,58 -147,166 0,63 21,118 48,153l79 -31c-19,-24 -39,-60 -39,-100 0,-55 30,-78 68,-78 56,1 109,52 201,155l56 60 66 0 0 -334 0 0zm-478 -370c-1,11 -1,24 1,44 3,74 27,136 69,183 47,50 122,83 210,83 113,0 206,-62 206,-187 0,-102 -76,-174 -171,-174 -97,0 -154,67 -154,145 0,45 14,78 39,101l0 2c-56,-12 -106,-55 -114,-154 -1,-17 -2,-30 -1,-43l-85 0 0 0zm405 125c0,50 -46,74 -95,76 -13,0 -22,-2 -28,-5 -24,-10 -42,-35 -42,-65 0,-48 38,-71 81,-71 48,0 84,25 84,65l0 0z"/>
                        <path class="fil8" id="text-27"
                              onclick="chooseTile(this)"
                              d="M23480 3962l-90 0 0 178 -2 0 -37 -44c-60,-67 -123,-126 -203,-126 -85,0 -147,60 -147,166 0,64 21,120 48,155l79 -31c-19,-25 -39,-61 -39,-101 0,-55 30,-78 68,-78 56,2 109,52 201,155l56 61 66 0 0 -335 0 0zm-471 -73l90 0 0 -217 1 0 380 195 0 -117 -402 -195 -69 0 0 334z"/>
                        <path class="fil8" id="text-28"
                              onclick="chooseTile(this)"
                              d="M24656 6895l-90 0 0 178 -2 0 -37 -43c-60,-68 -123,-126 -203,-126 -85,0 -148,60 -148,166 0,64 22,119 49,155l79 -31c-19,-25 -39,-60 -39,-102 0,-54 30,-77 68,-77 56,1 109,51 201,155l56 61 66 0 0 -336 0 0zm8 -235c0,-109 -57,-182 -143,-182 -61,0 -100,40 -117,87l-3 0c-23,-47 -63,-69 -104,-69 -57,0 -121,47 -121,156 0,95 53,167 133,167 41,0 80,-23 103,-68l2 0c23,51 62,84 118,84 65,0 132,-55 132,-175l0 0zm-76 -5c0,41 -33,67 -67,67 -34,0 -60,-21 -70,-57 12,-44 36,-74 78,-74 34,0 59,25 59,64l0 0zm-337 2c0,-40 29,-57 58,-57 29,0 52,23 61,48 -13,38 -33,67 -65,67 -30,0 -54,-20 -54,-58l0 0z"/>
                        <path class="fil8" id="text-29"
                              onclick="chooseTile(this)"
                              d="M24656 5433l-90 0 0 178 -2 0 -36 -44c-61,-68 -124,-126 -203,-126 -86,0 -149,60 -149,166 0,64 22,120 49,155l79 -31c-19,-25 -39,-61 -39,-102 0,-54 30,-77 69,-77 55,2 108,51 201,155l55 61 66 0 0 -335 0 0zm7 -103c0,-12 1,-31 0,-51 -3,-73 -26,-134 -66,-179 -47,-52 -120,-83 -217,-83 -111,0 -204,57 -204,178 0,105 78,178 171,178 91,0 146,-67 146,-147 0,-43 -12,-73 -34,-94l1 -2c35,8 67,28 88,59 17,25 26,59 29,92 1,23 1,35 0,49l86 0 0 0zm-406 -132c0,-50 49,-68 100,-68 13,0 21,2 27,5 17,10 31,31 31,61 0,45 -34,67 -75,67 -50,0 -83,-27 -83,-65z"/>
                        <path class="fil8" id="text-30"
                              onclick="chooseTile(this)"
                              d="M24631 4303c17,-27 33,-78 33,-140 0,-119 -61,-193 -145,-193 -61,0 -104,45 -114,103l-1 0c-21,-59 -61,-87 -110,-87 -65,0 -118,56 -118,157 0,60 18,118 36,146l79 -22c-11,-20 -28,-61 -28,-100 0,-46 21,-69 49,-69 40,0 55,47 56,84l0 43 78 0 0 -45c0,-49 21,-95 67,-95 36,0 64,29 64,86 0,46 -19,90 -30,110l84 22 0 0zm-455 -579c0,123 108,181 245,181 133,-1 243,-55 243,-177 0,-121 -100,-179 -246,-179 -130,0 -242,49 -242,175zm84 2c0,-44 54,-67 160,-67 106,0 161,24 161,66 0,43 -52,69 -160,68 -110,1 -161,-27 -161,-67z"/>
                        <path class="fil8" id="text-31"
                              onclick="chooseTile(this)"
                              d="M25810 7181c17,-27 34,-78 34,-139 0,-120 -62,-193 -145,-193 -62,0 -105,45 -115,102l-2 0c-20,-58 -59,-86 -109,-86 -65,0 -118,55 -118,156 0,61 18,118 36,146l80 -22c-11,-20 -29,-60 -29,-99 0,-47 22,-70 50,-70 40,0 54,47 54,85l0 42 79 0 0 -44c0,-50 22,-96 68,-96 36,0 63,29 63,87 0,45 -19,89 -29,109l83 22 0 0zm25 -547l0 -106 -472 0 0 91 58 125 83 -18 -42 -90 0 -2 373 0z"/>
                        <path class="fil8" id="text-32"
                              onclick="chooseTile(this)"
                              d="M25810 5763c17,-28 34,-79 34,-141 0,-119 -62,-193 -145,-193 -62,0 -105,46 -115,103l-2 0c-20,-59 -59,-87 -109,-87 -65,0 -118,57 -118,157 0,61 18,118 36,146l80 -22c-11,-20 -28,-61 -28,-99 0,-47 21,-70 49,-70 40,0 54,47 54,84l0 44 79 0 0 -46c0,-49 22,-95 68,-95 36,0 63,29 63,86 0,45 -19,91 -29,110l83 23 0 0zm25 -741l-90 0 0 178 -2 0 -36 -44c-61,-69 -125,-126 -203,-126 -87,0 -149,60 -149,166 0,64 22,120 49,155l79 -31c-18,-25 -40,-61 -40,-102 0,-54 32,-77 70,-77 55,2 108,51 200,155l55 61 67 0 0 -335 0 0z"/>
                        <path class="fil8" id="text-33"
                              onclick="chooseTile(this)"
                              d="M25810 4294c17,-27 34,-78 34,-140 0,-119 -62,-193 -145,-193 -62,0 -105,45 -115,103l-2 0c-20,-59 -59,-87 -109,-87 -65,0 -118,56 -118,157 0,60 18,118 36,146l80 -22c-11,-20 -29,-61 -29,-100 0,-46 22,-69 50,-69 40,0 54,47 54,84l0 43 79 0 0 -45c0,-49 22,-95 68,-95 36,0 63,29 63,86 0,45 -19,90 -29,110l83 22 0 0zm0 -403c17,-27 34,-79 34,-140 0,-119 -62,-193 -145,-193 -62,0 -105,45 -115,103l-2 0c-20,-59 -59,-87 -109,-87 -65,0 -118,56 -118,157 0,60 18,118 36,146l80 -23c-11,-19 -29,-60 -29,-99 0,-46 22,-69 50,-69 40,0 54,47 54,84l0 42 79 0 0 -44c0,-49 22,-95 68,-95 36,0 63,29 63,86 0,45 -19,90 -29,110l83 22 0 0z"/>
                        <path class="fil8" id="text-34"
                              onclick="chooseTile(this)"
                              d="M26989 7236c18,-28 34,-79 34,-141 0,-119 -61,-193 -144,-193 -63,0 -105,46 -116,103l-1 0c-20,-58 -59,-87 -109,-87 -65,0 -118,56 -118,157 0,62 17,118 35,146l81 -22c-12,-20 -29,-60 -29,-100 0,-46 22,-69 50,-69 39,0 53,47 54,84l0 44 79 0 0 -46c0,-49 22,-95 68,-95 35,0 63,29 63,86 0,46 -19,91 -29,110l82 23 0 0zm25 -707l-112 0 0 -56 -83 0 0 56 -276 0 0 136 288 178 71 0 0 -209 112 0 0 -105 0 0zm-195 212l-1 0 -103 -64c-30,-16 -57,-28 -87,-43l0 -3c30,2 59,3 87,3l104 0 0 107 0 0z"/>
                        <path class="fil8" id="text-35"
                              onclick="chooseTile(this)"
                              d="M26989 5762c18,-27 34,-78 34,-139 0,-120 -61,-193 -144,-193 -63,0 -105,45 -116,101l-1 0c-20,-57 -59,-85 -109,-85 -65,0 -118,55 -118,157 0,60 17,117 35,145l81 -23c-12,-19 -29,-60 -29,-98 0,-47 22,-70 50,-70 39,0 53,48 54,85l0 42 79 0 0 -45c0,-48 22,-95 68,-95 35,0 63,29 63,87 0,45 -19,89 -29,109l82 22 0 0zm-446 -720l0 259 247 32c-3,-20 -4,-38 -4,-64 0,-95 29,-133 79,-133 50,0 71,48 71,95 0,42 -13,85 -23,106l83 19c13,-26 27,-73 27,-130 0,-128 -78,-203 -165,-203 -56,0 -95,22 -119,57 -24,33 -34,79 -34,125 0,11 0,20 1,30l-72 -10 0 -183 -91 0 0 0z"/>
                        <path class="fil8" id="text-6"
                              onclick="chooseTile(this)"
                              d="M26989 4306c18,-28 34,-79 34,-141 0,-119 -61,-193 -144,-193 -63,0 -105,46 -116,103l-1 0c-20,-59 -59,-87 -109,-87 -65,0 -118,56 -118,157 0,61 17,118 35,146l81 -22c-12,-20 -29,-61 -29,-100 0,-46 22,-69 50,-69 39,0 53,47 54,84l0 44 79 0 0 -46c0,-49 22,-95 68,-95 35,0 63,29 63,86 0,46 -19,91 -29,110l82 23 0 0zm-453 -708c0,11 0,25 1,45 4,74 27,134 69,182 48,51 122,83 210,83 113,0 207,-61 207,-187 0,-103 -77,-175 -170,-175 -99,0 -155,67 -155,147 0,44 14,76 38,99l0 3c-55,-13 -105,-56 -114,-154 -1,-17 -2,-31 -1,-43l-85 0zm405 124c0,50 -44,74 -94,76 -13,0 -22,-1 -29,-4 -23,-11 -42,-36 -42,-65 0,-47 38,-70 82,-70 48,0 83,24 83,63l0 0z"/>

                        <path class="fil9"
                              d="M13721 9611l92 0 0 -406 -79 0 -107 50 15 71 78 -37 1 0 0 322zm251 -372l0 66 -40 0 0 70 40 0 0 126c0,43 9,72 27,90 15,16 40,27 71,27 27,0 49,-3 62,-8l-1 -72c-7,1 -16,2 -28,2 -29,0 -37,-17 -37,-54l0 -111 68 0 0 -70 -68 0 0 -87 -94 21zm360 59c-99,0 -164,64 -164,163 0,98 69,157 159,157 82,0 161,-51 161,-163 0,-92 -63,-157 -156,-157zm-3 68c43,0 61,46 61,92 0,55 -24,92 -61,92 -41,0 -63,-39 -63,-92 0,-45 17,-92 63,-92zm327 245l92 0 0 -406 -78 0 -108 50 16 71 77 -37 1 0 0 322zm371 7c94,0 156,-49 156,-123 0,-53 -35,-86 -74,-101l0 -2c40,-20 59,-55 59,-90 0,-49 -40,-104 -135,-104 -81,0 -143,45 -143,114 0,36 19,69 59,89l0 2c-45,19 -73,53 -73,102 0,55 48,113 151,113zm4 -65c-36,0 -59,-28 -58,-57 0,-30 18,-52 50,-61 37,10 63,31 63,67 0,29 -22,51 -55,51zm-2 -291c35,0 49,25 49,50 0,26 -19,46 -41,52 -33,-10 -58,-27 -58,-55 0,-26 17,-47 50,-47z"/>
                        <path class="fil9"
                              d="M25319 9605l92 0 0 -407 -79 0 -108 50 16 71 77 -37 2 0 0 323zm256 5c10,1 26,1 43,0 63,-2 116,-22 155,-57 44,-41 71,-102 71,-187 0,-95 -49,-175 -154,-175 -89,0 -153,67 -153,146 0,79 58,127 127,127 37,0 63,-10 81,-29l2 0c-7,31 -25,57 -51,76 -22,15 -51,23 -80,25 -19,1 -30,1 -41,0l0 74zm113 -350c43,0 59,43 59,86 0,12 -2,18 -5,24 -8,14 -27,27 -52,27 -39,0 -58,-30 -58,-65 -1,-43 23,-72 56,-72zm229 -28l0 66 -41 0 0 71 41 0 0 125c0,43 9,72 26,90 15,16 41,27 72,27 26,0 49,-3 62,-8l-1 -72c-7,1 -16,2 -28,2 -29,0 -38,-17 -38,-54l0 -110 68 0 0 -71 -68 0 0 -87 -93 21zm359 60c-98,0 -163,63 -163,162 0,99 69,157 158,157 82,0 162,-51 162,-163 0,-92 -63,-156 -157,-156zm-2 67c43,0 60,46 60,92 0,55 -23,92 -60,92 -41,0 -63,-39 -63,-92 0,-45 17,-92 63,-92zm203 224c24,15 68,28 121,28 103,0 166,-52 166,-124 0,-53 -39,-90 -88,-99l0 -1c50,-18 75,-52 75,-95 0,-56 -49,-101 -136,-101 -52,0 -101,15 -125,30l19 69c17,-10 52,-24 85,-24 40,0 60,18 60,42 0,35 -40,47 -72,47l-37 0 0 69 39 0c41,0 82,18 82,58 0,30 -26,54 -75,54 -39,0 -78,-16 -94,-25l-20 72zm610 -391c-10,-1 -21,-1 -38,1 -64,3 -117,23 -158,60 -43,40 -71,104 -71,181 0,97 53,177 161,177 88,0 150,-66 150,-147 0,-84 -57,-132 -126,-132 -38,0 -66,11 -86,32l-2 0c11,-47 48,-91 133,-98 15,-1 26,-1 37,-1l0 -73zm-107 349c-43,0 -64,-39 -66,-82 0,-11 1,-18 4,-24 9,-20 30,-36 56,-36 41,0 61,33 61,70 0,42 -21,72 -55,72z"/>
                        <path class="fil9"
                              d="M23407 9175c-129,0 -213,98 -213,229 0,125 76,223 206,223 128,0 215,-87 215,-230 0,-121 -74,-222 -208,-222zm-3 79c68,0 106,67 106,146 0,85 -38,148 -105,148 -66,0 -106,-60 -106,-146 0,-85 38,-148 105,-148zm276 364c24,3 61,6 112,6 86,0 156,-18 202,-56 42,-36 72,-94 72,-177 0,-78 -28,-131 -74,-165 -42,-32 -96,-47 -179,-47 -50,0 -97,3 -133,9l0 430zm99 -359c8,-2 23,-4 45,-4 85,0 138,48 137,139 0,104 -58,154 -148,153 -12,0 -26,0 -34,-2l0 -286zm352 359c25,3 61,6 113,6 85,0 156,-18 202,-56 42,-36 72,-94 72,-177 0,-78 -28,-131 -74,-165 -42,-32 -96,-47 -179,-47 -50,0 -97,3 -134,9l0 430zm100 -359c8,-2 22,-4 45,-4 85,0 138,48 137,139 0,104 -58,154 -148,153 -12,0 -26,0 -34,-2l0 -286z"/>
                        <path class="fil9"
                              d="M16251 9354l-161 0 0 -91 170 0 0 -81 -270 0 0 438 279 0 0 -81 -179 0 0 -104 161 0 0 -81zm309 266l148 -438 -105 0 -56 187c-15,50 -29,102 -40,155l-2 0c-10,-55 -24,-105 -39,-157l-53 -185 -108 0 140 438 115 0zm455 -266l-161 0 0 -91 170 0 0 -81 -270 0 0 438 280 0 0 -81 -180 0 0 -104 161 0 0 -81zm177 266l0 -132c0,-71 -1,-133 -4,-191l2 0c21,51 50,107 76,154l93 169 104 0 0 -438 -91 0 0 128c0,66 2,125 9,183l-2 0c-20,-50 -45,-104 -71,-150l-91 -161 -116 0 0 438 91 0z"/>
                        <path class="fil9"
                              d="M14797 8443l125 0 0 -551 -107 0 -146 68 22 96 105 -50 1 0 0 437zm898 0l124 0 0 -551 -107 0 -145 68 21 96 105 -50 2 0 0 437zm696 0l0 -106 -207 0 0 -2 51 -42c79,-72 146,-145 146,-238 0,-100 -68,-173 -193,-173 -75,0 -139,26 -181,57l37 93c29,-22 70,-46 118,-46 63,0 90,35 90,80 -1,65 -60,127 -180,234l-71 65 0 78 390 0zm-1307 -261c17,10 45,16 75,16 68,0 102,-32 102,-75 0,-35 -19,-57 -64,-73 -28,-10 -38,-16 -38,-27 0,-12 11,-19 28,-19 20,0 41,8 51,13l13 -50c-14,-7 -39,-13 -66,-13 -58,0 -95,33 -96,76 0,28 19,55 67,71 27,9 35,15 35,27 0,12 -9,20 -32,20 -21,0 -49,-10 -62,-18l-13 52zm231 -274l0 51 -31 0 0 54 31 0 0 96c0,33 7,55 21,69 11,12 31,20 54,20 21,0 38,-2 48,-6l-1 -55c-5,1 -12,2 -21,2 -22,0 -29,-13 -29,-41l0 -85 52 0 0 -54 -52 0 0 -67 -72 16z"/>
                        <path class="fil9"
                              d="M19756 8443l0 -106 -208 0 0 -2 51 -42c80,-72 147,-145 147,-238 0,-100 -69,-173 -193,-173 -75,0 -139,26 -181,57l37 93c28,-22 70,-46 117,-46 64,0 91,35 91,80 -2,65 -60,127 -180,234l-72 65 0 78 391 0zm854 0l125 0 0 -551 -107 0 -146 68 21 96 105 -50 2 0 0 437zm697 0l0 -106 -208 0 0 -2 51 -42c80,-72 147,-145 147,-238 0,-100 -69,-173 -194,-173 -74,0 -139,26 -180,57l36 93c29,-22 71,-46 118,-46 64,0 91,35 91,80 -2,65 -60,127 -181,234l-71 65 0 78 391 0zm-1433 -210l73 0 0 -136c0,-7 1,-14 3,-18 5,-13 17,-27 37,-27 26,0 37,20 37,50l0 131 73 0 0 -140c0,-69 -36,-100 -84,-100 -39,0 -63,22 -73,37l-1 0 -3 -32 -64 0c1,20 2,45 2,75l0 160zm436 -341l0 127 -1 0c-11,-16 -33,-26 -62,-26 -56,0 -105,45 -105,124 0,73 45,121 100,121 31,0 59,-14 74,-40l1 0 3 35 65 0c-1,-16 -2,-44 -2,-71l0 -270 -73 0zm0 236c0,6 -1,11 -2,17 -4,20 -21,35 -42,35 -30,0 -50,-25 -50,-65 0,-37 17,-67 50,-67 23,0 39,16 43,36 1,4 1,9 1,13l0 31z"/>
                        <path class="fil9"
                              d="M24069 8408c32,21 92,39 163,39 140,0 226,-71 226,-168 0,-72 -54,-122 -120,-134l0 -2c68,-24 101,-69 101,-128 0,-75 -65,-137 -183,-137 -71,0 -137,20 -170,41l26 94c23,-14 70,-34 115,-34 54,0 81,25 81,58 0,47 -55,63 -98,64l-50 0 0 92 53 0c56,0 111,25 111,79 0,41 -34,74 -101,74 -53,0 -105,-22 -128,-34l-26 96zm1145 30l125 0 0 -551 -107 0 -146 68 21 97 105 -50 2 0 0 436zm697 0l0 -106 -208 0 0 -2 51 -42c80,-71 147,-145 147,-237 0,-100 -69,-173 -194,-173 -74,0 -139,25 -180,57l36 92c29,-22 71,-46 118,-46 64,0 91,36 91,81 -2,64 -60,126 -181,234l-71 64 0 78 391 0zm-1341 -210l73 0 0 -119c0,-6 0,-12 1,-17 5,-22 23,-36 50,-36 8,0 15,0 20,1l0 -68c-5,-1 -9,-1 -15,-1 -23,0 -52,14 -64,48l-2 0 -2 -43 -63 0c1,20 2,43 2,77l0 158zm333 -341l0 127 -1 0c-11,-16 -33,-26 -62,-26 -56,0 -105,45 -105,125 0,73 45,120 100,120 31,0 59,-13 74,-40l1 0 3 35 65 0c-1,-16 -2,-44 -2,-70l0 -271 -73 0zm0 236c0,6 -1,12 -2,17 -4,21 -21,35 -42,35 -30,0 -50,-25 -50,-65 0,-37 17,-66 50,-66 23,0 39,16 43,36 1,3 1,9 1,12l0 31z"/>
                        <path class="fil9"
                              d="M28176 6927l-78 0 0 152 -1 0 -31 -37c-52,-58 -106,-107 -174,-107 -73,0 -127,50 -127,141 0,55 19,102 42,133l68 -27c-17,-21 -34,-52 -34,-86 0,-47 26,-67 59,-67 47,1 93,44 171,132l48 53 57 0 0 -287zm-194 -133c0,-34 -24,-56 -57,-56 -34,0 -57,22 -57,55 0,33 24,56 57,56 33,0 57,-23 57,-55zm201 0c0,-34 -25,-56 -57,-56 -33,0 -57,22 -57,55 0,33 24,56 57,56 32,0 57,-23 57,-55zm-7 -207l0 -91 -404 0 0 78 50 107 71 -16 -37 -77 0 -1 320 0z"/>
                        <path class="fil9"
                              d="M28176 5465l-78 0 0 152 -1 0 -31 -37c-52,-58 -106,-107 -174,-107 -73,0 -127,50 -127,141 0,55 19,102 42,133l68 -27c-17,-21 -34,-52 -34,-87 0,-46 26,-66 59,-66 47,1 93,44 171,132l48 52 57 0 0 -286zm-194 -133c0,-34 -24,-56 -57,-56 -34,0 -57,22 -57,55 0,33 24,56 57,56 33,0 57,-23 57,-55zm201 0c0,-34 -25,-56 -57,-56 -33,0 -57,22 -57,55 0,33 24,56 57,56 32,0 57,-23 57,-55zm-7 -207l0 -91 -404 0 0 78 50 107 71 -16 -37 -77 0 -1 320 0z"/>
                        <path class="fil9"
                              d="M28176 3999l-78 0 0 152 -1 0 -31 -37c-52,-59 -106,-108 -174,-108 -73,0 -127,50 -127,142 0,54 19,102 42,132l68 -27c-17,-21 -34,-51 -34,-86 0,-47 26,-66 59,-66 47,1 93,44 171,132l48 52 57 0 0 -286zm-194 -134c0,-33 -24,-56 -57,-55 -34,0 -57,22 -57,55 0,33 24,56 57,56 33,0 57,-23 57,-56zm201 0c0,-33 -25,-56 -57,-55 -33,0 -57,22 -57,55 0,33 24,56 57,56 32,0 57,-23 57,-56zm-7 -207l0 -91 -404 0 0 78 50 107 71 -15 -37 -77 0 -2 320 0z"/>

                        <path class="fil5"
                              d="M27412 7639l0 1985c0,140 -56,274 -156,373 -99,100 -233,156 -373,156l-13227 0c-141,0 -274,-56 -374,-156 -99,-99 -155,-233 -155,-373l0 -1985c-52,1 -105,1 -157,2 -107,1 -217,4 -323,-5 -78,-6 -156,-20 -226,-57 -59,-31 -107,-75 -148,-127 -72,-91 -128,-208 -176,-313 -35,-78 -69,-157 -102,-237 -41,-100 -82,-202 -123,-303 -70,-174 -140,-347 -212,-521 -93,-225 -188,-449 -286,-673l-7 -17 7 -17c98,-224 193,-448 286,-673 72,-173 142,-347 212,-521 41,-101 82,-202 123,-303 33,-79 67,-159 102,-237 48,-105 104,-222 176,-312 41,-52 89,-96 148,-128 70,-37 148,-50 226,-57 106,-9 216,-6 323,-4 60,0 121,1 182,1l14944 0c135,0 264,54 359,149 96,96 149,224 149,360l0 3485c0,135 -53,263 -149,359 -95,95 -224,149 -359,149l-684 0zm-87 1040l0 -1040 -4631 0 0 1040 4631 0zm-2272 1387l1830 0c117,0 229,-47 312,-130 83,-83 130,-195 130,-312l0 -858 -2272 0 0 1300zm-2359 0l2272 0 0 -1300 -2272 0 0 1300zm-2335 0l2248 0 0 -1300 -2248 0 0 1300zm-2383 0l2296 0 0 -1300 -2296 0 0 1300zm-2359 0l2272 0 0 -1300 -2272 0 0 1300zm-2403 -1300l0 858c0,117 47,229 130,312 83,83 194,130 312,130l1874 0 0 -1300 -2316 0zm0 -1127l0 1040 4675 0 0 -1040 -4675 0zm-87 -86l0 -4329c-53,-1 -105,-1 -158,-2 -58,-1 -116,-1 -173,-1 -47,0 -94,1 -140,5 -69,6 -139,19 -201,53 -45,25 -82,59 -114,99 -31,39 -57,81 -80,124 -32,56 -60,115 -87,174 -34,77 -67,155 -100,233 -41,100 -81,200 -121,300 -70,173 -140,346 -211,518 -92,221 -185,442 -281,661 96,220 189,440 281,661 71,172 141,345 211,518 40,100 80,200 121,300 33,78 66,156 100,233 27,59 55,118 87,175 23,42 49,85 80,123 32,40 69,75 114,100 62,34 132,46 201,52 46,4 93,5 140,5 57,1 115,0 173,-1 53,0 105,-1 158,-1zm4849 1126l4631 0 0 -1040 -4631 0 0 1040zm-4762 -2609l1137 0 0 -1355 -1137 0 0 1355zm0 -1442l1137 0 0 -1404 -1137 0 0 1404zm1223 -1404l0 1404 1093 0 0 -1404 -1093 0zm1180 0l0 1404 1093 0 0 -1404 -1093 0zm1179 0l0 1404 1093 0 0 -1404 -1093 0zm1180 0l0 1404 1093 0 0 -1404 -1093 0zm1179 0l0 1404 1093 0 0 -1404 -1093 0zm1180 0l0 1404 1093 0 0 -1404 -1093 0zm1179 0l0 1404 1093 0 0 -1404 -1093 0zm1180 0l0 1404 1093 0 0 -1404 -1093 0zm1179 0l0 1404 1093 0 0 -1404 -1093 0zm1180 0l0 1404 1093 0 0 -1404 -1093 0zm1179 0l0 1404 1093 0 0 -1404 -1093 0zm1180 0l0 1404 1106 0 0 -982c0,-113 -45,-219 -124,-298 -79,-80 -186,-124 -298,-124l-684 0zm1106 1491l-1106 0 0 1355 1106 0 0 -1355zm0 1442l-1106 0 0 1396 684 0c112,0 219,-45 298,-124 79,-80 124,-186 124,-298l0 -974zm-1193 1396l0 -1396 -1093 0 0 1396 1093 0zm-1179 0l0 -1396 -1093 0 0 1396 1093 0zm-1180 0l0 -1396 -1093 0 0 1396 1093 0zm-1179 0l0 -1396 -1093 0 0 1396 1093 0zm-1180 0l0 -1396 -1093 0 0 1396 1093 0zm-1179 0l0 -1396 -1093 0 0 1396 1093 0zm-1180 0l0 -1396 -1093 0 0 1396 1093 0zm-1179 0l0 -1396 -1093 0 0 1396 1093 0zm-1180 0l0 -1396 -1093 0 0 1396 1093 0zm-1179 0l0 -1396 -1093 0 0 1396 1093 0zm-1180 0l0 -1396 -1093 0 0 1396 1093 0zm-1179 0l0 -1396 -1137 0 0 1396 1137 0zm11881 -1483l1093 0 0 -1355 -1093 0 0 1355zm-1179 0l1093 0 0 -1355 -1093 0 0 1355zm-1180 0l1093 0 0 -1355 -1093 0 0 1355zm-1179 0l1093 0 0 -1355 -1093 0 0 1355zm-1180 0l1093 0 0 -1355 -1093 0 0 1355zm-1179 0l1093 0 0 -1355 -1093 0 0 1355zm-1180 0l1093 0 0 -1355 -1093 0 0 1355zm-1179 0l1093 0 0 -1355 -1093 0 0 1355zm-1180 0l1093 0 0 -1355 -1093 0 0 1355zm-1179 0l1093 0 0 -1355 -1093 0 0 1355zm-1180 0l1093 0 0 -1355 -1093 0 0 1355z"/>
                    </g>
                </g>
            </g>
</svg>
    </div>

    <div
        class="modal opacity-0 pointer-events-none fixed w-full h-full top-0 left-0 flex items-center justify-center">
        <div
            class="modal-overlay absolute w-full h-full bg-gray-900 opacity-50"></div>

        <div
            class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">

            <div
                class="modal-close absolute top-0 right-0 cursor-pointer flex flex-col items-center mt-4 mr-4 text-white text-sm z-50">
                <svg class="fill-current text-white"
                     xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                     viewBox="0 0 18 18">
                    <path
                        d="M14.53 4.53l-1.06-1.06L9 7.94 4.53 3.47 3.47 4.53 7.94 9l-4.47 4.47 1.06 1.06L9 10.06l4.47 4.47 1.06-1.06L10.06 9z"></path>
                </svg>
                <span class="text-sm">(Esc)</span>
            </div>

            <!-- Add margin if you want to see some of the overlay behind the modal-->
            <div class="modal-content py-4 text-left px-6">
                <!--Title-->
                <div class="flex justify-between items-center pb-3">
                    <p class="text-2xl font-bold">Faites vos jeux !</p>
                    <div class="modal-close cursor-pointer z-50">
                        <svg class="fill-current text-black"
                             xmlns="http://www.w3.org/2000/svg" width="18"
                             height="18"
                             viewBox="0 0 18 18">
                            <path
                                d="M14.53 4.53l-1.06-1.06L9 7.94 4.53 3.47 3.47 4.53 7.94 9l-4.47 4.47 1.06 1.06L9 10.06l4.47 4.47 1.06-1.06L10.06 9z"></path>
                        </svg>
                    </div>
                </div>

                <!--Body-->
                <label for="mise">Paris sur le <span id="display-amount"></span></label><br/>
                <input id="mise" type="number" class="bg-gray-300 text-black"
                       value="1"/><br/>
                <label><span id="display-jeton"></span> jetons
                    disponible</label>

                <!--Footer-->
                <div class="flex justify-end pt-2">
                    <button id="btn_validate"
                            class="modal-close px-4 bg-red-800 p-3 rounded-lg text-white hover:bg-red-600">
                        Ajouter
                    </button>
                </div>

            </div>
        </div>
    </div>

    <button id="btn_submit_bet" onclick="submit_bets()" class="fixed
    bg-gray-500
    hover:bg-gray-400 text-white
     font-bold py-2 px-4 border-b-4 border-gray-700 hover:border-gray-500
     rounded right-10 bottom-10">Valider
    </button>

    <script>
        let bet = {};
        let playerJeton = 0;
        let actualbet = null;
        let validate = false;
        @if ($participant)
        let participate = true;
        @else
        let participate = false;
        @endif

        window.addEventListener("load", function (event) {
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function () {
                if (this.readyState === 4 && this.status === 200) {
                    playerJeton = this.responseText;
                    console.log(playerJeton);
                }
            }
            xhttp.open("GET", '{{route('auth.wizebot.jeton')}}', true);
            xhttp.send();
        })

        function SitAtTable() {
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function () {
                if (this.readyState === 4 && this.status === 200) {
                    switch (this.responseText) {
                        case "0":
                            alert('Vous etes à present à table !');
                            document.getElementById('btn_participate').hidden = true;
                            participate = true;
                            break;
                        case "2":
                            alert('A trop tard ! Aucun siège disponible')
                            participate = false;
                            break;
                        case "3":
                            alert('Vous faites déjà partie de la table');
                            participate = true;
                            break;
                    }
                }
            }
            xhttp.open("GET", '{{route('room.sit',
                array('roomid' => $idRoom))}}',
                true);
            xhttp.send();
        }

        function submit_bets() {
            const obj = JSON.stringify(bet);
            btn = document.getElementById('btn_submit_bet');
            btn.disabled = true;
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function () {
                if (this.readyState === 4 && this.status === 200) {
                    console.log(this.responseText);
                }
            }
            xhttp.open("GET", '{{route('room.submit_bet')}}?bets=' +
                encodeURIComponent(obj) + '&gameid=' + {{$idRoom}},
                true);
            xhttp.send();
        }

        function chooseTile(elmt) {
            if (participate) {
                // gestion des text-id
                if (elmt.id.toString().startsWith('text')) {
                    elmt.id = elmt.id.toString().split('-')[1]
                }

                // Test if exist
                manageJetons(elmt);
            } else {
                alert("Pense à t'assoir à la table si tu veux jouer :) ");
            }
        }

        function PopUpAmount(elmt) {
            toggleModal(elmt)
        }

        function manageJetons(elmt) {
            if (bet[elmt.id] === undefined) {
                PopUpAmount(elmt)
                actualbet = elmt;
            } else {
                if (confirm('Êtes-vous sûr(e) de supprimer le jeton ?')) {
                    var jeton = document.getElementById('bet' + elmt.id)
                    jeton.parentNode.removeChild(jeton);
                    delete bet[elmt.id];
                }
            }
        }

        function placeJetons(elmt) {
            let bound = elmt.getBBox();
            let newX = (bound.x + bound.width / 2);
            let newY = (bound.y + bound.height / 2 - 400);

            var jeton = document.createElementNS('http://www.w3.org/2000/svg', 'g');
            jeton.innerHTML = '<path class="fil5" d="M0 0c213,0 387,173 387,387 0,213 -174,386 -387,386 -214,0 -387,-173 -387,-386 0,-214 173,-387 387,-387z"></path>' +
                '<path class="fil16" d="M0 50c191,0 346,154 346,346 0,191 -155,346 -346,346 -191,0 -346,-155 -346,-346 0,-192 155,-346 346,-346zm0 -23c102,0 194,41 261,108 66,66 108,159 108,261 0,101 -42,194 -108,260 -67,67 -159,108 -261,108 -102,0 -194,-41 -261,-108 -67,-66 -108,-159 -108,-260 0,-102 41,-195 108,-261 67,-67 159,-108 261,-108zm255 114c-66,-66 -156,-106 -255,-106 -100,0 -190,40 -255,106 -65,65 -106,155 -106,255 0,99 41,189 106,254 65,66 155,106 255,106 99,0 189,-40 255,-106 65,-65 105,-155 105,-254 0,-100 -40,-190 -105,-255zm-255 57c54,0 104,22 139,58 36,36 58,85 58,140 0,54 -22,103 -58,139 -35,36 -85,58 -139,58 -55,0 -104,-22 -140,-58 -36,-36 -58,-85 -58,-139 0,-55 22,-104 58,-140 36,-36 85,-58 140,-58zm123 74c-31,-31 -75,-51 -123,-51 -48,0 -92,20 -123,51 -32,32 -52,75 -52,124 0,48 20,91 52,123 31,31 75,51 123,51 48,0 92,-20 123,-51 32,-32 51,-75 51,-123 0,-49 -19,-92 -51,-124zm-52 -196c-4,36 -34,64 -71,64 -37,0 -68,-28 -72,-64 23,-5 47,-8 72,-8 24,0 48,3 71,8zm205 144c-28,22 -69,21 -95,-5 -26,-27 -28,-68 -5,-96 40,26 75,60 100,101zm44 247c-36,-4 -64,-34 -64,-71 0,-37 28,-68 64,-72 5,23 8,47 8,72 0,24 -3,48 -8,71zm-144 205c-23,-28 -21,-69 5,-95 26,-27 67,-28 95,-6 -25,41 -60,75 -100,101zm-176 -21c37,0 67,28 71,64 -23,6 -47,8 -71,8 -25,0 -49,-2 -72,-8 4,-36 35,-64 72,-64zm-181 -74c26,26 28,67 5,95 -40,-26 -75,-60 -101,-101 28,-22 70,-21 96,6zm-75 -181c0,37 -28,67 -64,71 -5,-23 -8,-47 -8,-71 0,-25 3,-49 8,-72 36,4 64,35 64,72zm75 -181c-26,26 -68,27 -96,5 26,-41 61,-75 101,-101 23,28 21,69 -5,96zm181 -43c124,0 224,100 224,224 0,123 -100,224 -224,224 -124,0 -224,-101 -224,-224 0,-124 100,-224 224,-224z"></path>'
            jeton.setAttribute('style', "transform:translate(" + newX + "px, " + newY + "px);")
            jeton.id = 'bet' + elmt.id
            document.getElementById('parent').appendChild(jeton);
        }

        function getMyMise() {
            return document.getElementById('mise').value;
        }

        const overlay = document.querySelector('.modal-overlay')
        overlay.addEventListener('click', toggleModal)

        let closemodal = document.querySelectorAll('.modal-close')
        for (let i = 0; i < closemodal.length; i++) {
            if (closemodal[i].id === 'btn_validate') {
                closemodal[i].addEventListener('click', toggleModal_Validate)
            } else {
                closemodal[i].addEventListener('click', toggleModal)
            }
        }

        document.onkeydown = function (evt) {
            evt = evt || window.event
            var isEscape = false
            if ("key" in evt) {
                isEscape = (evt.key === "Escape" || evt.key === "Esc")
            } else {
                isEscape = (evt.keyCode === 27)
            }
            if (isEscape && document.body.classList.contains('modal-active')) {
                toggleModal()
            }
        };

        function toggleModal_Validate(elmt) {
            validate = true;
            toggleModal(elmt)
        }

        function toggleModal(elmt) {
            const body = document.querySelector('body')
            const modal = document.querySelector('.modal')
            modal.classList.toggle('opacity-0')
            modal.classList.toggle('pointer-events-none')
            body.classList.toggle('modal-active')

            const title = document.querySelector('#display-amount')
            title.innerHTML = elmt.id

            const jeton = document.querySelector('#display-jeton')
            jeton.innerHTML = playerJeton

            if (validate) {
                var miseWanted = document.querySelector('#mise').value;
                if (playerJeton - miseWanted >= 0) {
                    bet[actualbet.id] = miseWanted
                    placeJetons(actualbet);
                    actualbet = null;
                    playerJeton -= miseWanted;
                    document.querySelector('#mise').value = 1;
                } else {

                }
            }

            validate = false;
        }

    </script>
</x-guest-layout>
